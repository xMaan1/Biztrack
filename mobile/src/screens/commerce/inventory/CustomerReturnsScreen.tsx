...
export default function CustomerReturnsScreen() {
  ...
  const loadReturns = async () => {
    try {
      setLoading(true);
      const response = await InventoryService.getCustomerReturns();
      ...
      Alert.alert('Error', error.message || 'Failed to load customer returns');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <View style={styles.returnDetails}>
        <View style={styles.detailRow}>
          <Ionicons name="arrow-back-outline" size={16} color={colors.green[600]} />
          <Text style={styles.detailText}>Quantity: {item.quantity}</Text>
        </View>
        <View style={styles.detailRow}>
          <Ionicons name="cash-outline" size={16} color={colors.text.secondary} />
          <Text style={styles.detailText}>
            Refund Value: {formatCurrency(item.quantity * item.unitCost)}
          </Text>
        </View>
        {item.notes && (
          <View style={styles.detailRow}>
    ...
    <View>
      ...
      <View style={styles.statsContainer}>
        ...
        <View style={styles.statCard}>
          ...
          <Text style={styles.statLabel}>Total Refund Value</Text>
        </View>
      </View>
    </View>
  );
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer}>
      <Ionicons name="arrow-back-outline" size={64} color={colors.text.secondary} />
      <Text style={styles.emptyText}>No customer returns found</Text>
    </View>
  );
