...
export default function QualityCheckListScreen() {
  ...
  const loadQualityChecks = async () => {
    try {
      ...
      if (typeFilter !== 'all') filters.inspection_type = typeFilter;
      if (searchTerm) filters.search = searchTerm;

      const response = await QualityControlService.getQualityChecks(
        filters,
        currentPage,
        itemsPerPage,
      );
      
      let filtered = response.quality_checks || [];
      ...
      if (currentPage === 1) {
        setQualityChecks(filtered);
      } else {
        setQualityChecks([...qualityChecks, ...filtered]);
      }
      setTotalPages(response.total_pages || 1);
    } catch (error: any) {
      Alert.alert('Error', error.message || 'Failed to load quality checks');
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderQualityCheckItem = ({ item }: { item: QualityCheck }) => {
    const statusStyle = getQualityStatusColor(item.status);
    const priorityStyle = getQualityPriorityColor(item.priority);

    return (
      <TouchableOpacity
        style={styles.qualityCheckCard}
        onPress={() => (navigation.navigate as any)('QualityCheckDetail', { id: item.id })}
      >
        <View style={styles.qualityCheckHeader}>
          <View style={styles.qualityCheckInfo}>
            <View style={styles.titleRow}>
              <Text style={styles.qualityCheckId}>{item.id}</Text>
            ...
            <Text style={styles.qualityCheckTitle}>{item.title}</Text>
            {item.description && (
              <Text style={styles.qualityCheckDescription} numberOfLines={2}>
                {item.description}
              </Text>
            )}
          </View>
        </View>
        <View style={styles.qualityCheckFooter}>
          <View style={styles.badgeRow}>
            <View style={styles.typeBadge}>
              <Text style={styles.typeIcon}>{getInspectionTypeIcon(item.inspection_type)}</Text>
              <Text style={styles.typeText}>{getInspectionTypeLabel(item.inspection_type)}</Text>
            </View>
          ...
          <View style={styles.detailsRow}>
            {item.scheduled_date && (
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailText}>
                  {QualityControlService.formatDate(item.scheduled_date)}
                </Text>
              </View>
            )}
            {item.estimated_duration_minutes > 0 && (
              <View style={styles.detailItem}>
                ...
                <Text style={styles.detailText}>{item.estimated_duration_minutes}m</Text>
              </View>
            )}
            {item.completion_percentage > 0 && (
              <View style={styles.detailItem}>
                <Ionicons name="checkmark-circle-outline" size={16} color={colors.text.secondary} />
                <Text style={styles.detailText}>{item.completion_percentage}%</Text>
              </View>
            )}
          </View>
          {item.completion_percentage > 0 && (
      ...
      {showFilters && (
        <View style={styles.filtersContainer}>
            ...
            <View style={styles.filterOptions}>
              {['all', 'low', 'medium', 'high', 'critical'].map((priority) => (
                ...
                >
                  ...
                  >
                    {priority === 'all' ? 'All' : getQualityPriorityLabel(priority as QualityPriority)}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.filterRow}>
            ...
            <View style={styles.filterOptions}>
              {['all', 'visual', 'dimensional', 'functional', 'material', 'safety', 'environmental', 'documentation'].map((type) => (
                <TouchableOpacity
                  key={type}
                  style={[
                    styles.filterChip,
                ...
                >
                  ...
                  >
                    {type === 'all' ? 'All' : getInspectionTypeLabel(type as InspectionType)}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.filterRow}>
            ...
            <View style={styles.filterOptions}>
              {['newest', 'oldest', 'priority', 'scheduled_date'].map((sort) => (
                <TouchableOpacity
                  key={sort}
                  style={[
                    styles.filterChip,
      ...
      {!searchTerm && activeTab === 'all' && statusFilter === 'all' && priorityFilter === 'all' && typeFilter === 'all' && (
        <>
          ...
          >
            ...
            <Text style={styles.createButtonText}>Create Quality Check</Text>
          </TouchableOpacity>
        </>
      )}
    </View>
  ...
  return (
    <Container safeArea>
      <Header
        ...
        gradient={false}
        rightIcon="add"
        onRightPress={() => (navigation.navigate as any)('QualityCheckForm', {})}
      />
      <FlatList
        data={qualityChecks}
        renderItem={renderQualityCheckItem}
