  ...
  return (
    <View className="flex-1 bg-slate-50">
      ...
      <View className="flex-row border-b border-slate-200 bg-white px-2 py-2">
        <Pressable
          onPress={() => setTab('list')}
          className={`mr-2 flex-1 rounded-lg py-2 ${
            tab === 'list' ? 'bg-blue-600' : 'bg-slate-100'
          }`}
        >
          <Text
            className={`text-center text-sm font-semibold ${
              tab === 'list' ? 'text-white' : 'text-slate-700'
            }`}
          >
            List
          </Text>
        ...
        <Pressable
          ...
          className={`flex-1 rounded-lg py-2 ${
            tab === 'dashboard' ? 'bg-blue-600' : 'bg-slate-100'
          }`}
        >
          <Text
            className={`text-center text-sm font-semibold ${
              tab === 'dashboard' ? 'text-white' : 'text-slate-700'
            }`}
          >
            Dashboard
          </Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                >
                  ...
                  <Text className="mt-1 text-slate-800">{item.customerName}</Text>
                  <View className="mt-2 flex-row justify-between">
                    <Text className="text-slate-600">{String(item.status)}</Text>
                    <Text className="font-semibold text-slate-900">
        ...
        >
          {dashboard ? (
            <View className="rounded-xl border border-slate-200 bg-white p-4">
              <Text className="text-lg font-semibold text-slate-900">Metrics</Text>
              <Text className="mt-2 text-slate-700">
                Total {dashboard.metrics.totalInvoices} · Paid{' '}
                {dashboard.metrics.paidInvoices} · Overdue{' '}
                {dashboard.metrics.overdueInvoices}
              </Text>
              <Text className="mt-1 text-slate-700">
                Revenue {formatUsd(dashboard.metrics.totalRevenue)} · Outstanding{' '}
                {formatUsd(dashboard.metrics.outstandingAmount)}
              </Text>
              <Text className="mt-3 font-semibold text-slate-900">Recent</Text>
              {(dashboard.recentInvoices ?? []).slice(0, 5).map((inv) => (
                ...
                >
                  ...
                  <Text className="text-slate-600">{formatUsd(inv.total)}</Text>
                </Pressable>
              ))}
            </View>
          ) : (
      ...
      <Modal visible={detailOpen} animationType="slide">
        <View className="flex-1 bg-white">
          <View className="flex-row items-center justify-between border-b border-slate-200 px-3 py-3">
            ...
            <Text className="text-lg font-semibold">Invoice</Text>
            <View className="w-12" />
          </View>
          <ScrollView className="flex-1 px-4 py-3">
            {selected ? (
              <>
                ...
                <Text className="mt-1 text-slate-800">{selected.customerName}</Text>
                <Text className="mt-1 text-slate-600">{selected.customerEmail}</Text>
                <Text className="mt-3 text-lg font-semibold text-slate-900">
                  {formatUsd(selected.total)} · {String(selected.status)}
                </Text>
                <View className="mt-4 flex-row flex-wrap gap-2">
                  {canEdit ? (
                    <>
                      ...
                      >
                        <Text className="font-semibold text-white">Mark paid</Text>
                      </Pressable>
                      <Pressable
                        onPress={() => {
