...
import { useStudentApplications } from '@/lib/hooks/useApplications'
import { Search, Clock, Eye, CheckCircle, XCircle, AlertCircle } from 'lucide-react'
import type { StudentApplication } from '@/lib/types/application.types'
...
export default function AdminStudentApplicationsPage() {
  ...
  const { data, isLoading } = useStudentApplications({ page, page_size: 10, status_filter: statusFilter || undefined, search: search || undefined })

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Student Applications</h1>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700/50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Interested Courses</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {data.applications.map((app: StudentApplication) => {
                  const a = app
                  return (
                    <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">{a.current_qualification || 'N/A'}</td>
                      <td className="px-6 py-4 text-sm text-gray-500">
                        {Array.isArray(a.interested_courses) ? a.interested_courses.join(', ') : a.interested_courses || 'N/A'}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/admin/applications/students/${a.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-lg text-sm font-medium hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors">
