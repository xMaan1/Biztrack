...
import { useMyTeacherApplication, useTeacherApplicationDetail } from '@/lib/hooks/useApplications'
import { useRouter } from 'next/navigation'
import { Clock, CheckCircle, XCircle, AlertCircle, ArrowLeft, FileText } from 'lucide-react'
...
export default function TeacherApplicationDetailPage() {
  ...
  const { data: myApp } = useMyTeacherApplication()
  const { data, isLoading } = useTeacherApplicationDetail(myApp?.id || 0)
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              {[
                ...
                ['Experience', application.teaching_experience],
                ['Current Job', application.current_job],
              ].map(([label, value]) => (
                <div key={label}>
                  <p className="text-gray-500">{label}</p>
                  <p className="font-medium text-gray-900 dark:text-white">{value || 'N/A'}</p>
